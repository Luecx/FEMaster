...
class LinearStaticStep:
    """Represents a linear-static analysis step with load/support collectors."""

    loadcase_type = "LINEAR STATIC"
    ...
    def add_load_collector(self, collector: LoadCollector) -> LoadCollector:
        if not isinstance(collector, LoadCollector):
            raise TypeError("LinearStaticStep only accepts LoadCollector instances.")
    ...
    def add_support_collector(self, collector: SupportCollector) -> SupportCollector:
        if not isinstance(collector, SupportCollector):
            raise TypeError("LinearStaticStep only accepts SupportCollector instances.")
    ...
    def to_asami(self) -> str:
        raise NotImplementedError("LinearStaticStep.to_asami not implemented yet.")
