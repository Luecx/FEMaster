...
\section{Node-to-surface contact between two C3D8 blocks}

This regression model isolates the current node-to-surface penalty formulation. The lower block is fixed. The upper block starts with a 0.5 gap, is guided laterally, and receives a prescribed downward displacement at its top face. The four nodes of the upper-block S1 face are the slave contact nodes; the lower-block S2 face is the master face. Contact begins when the initial gap has closed.

\begin{exampledeck}
*MODEL, NAME=CONTACT_NODE_SURFACE_PARALLEL_BLOCKS_C3D8
*NODE, NSET=NALL
1,  0.0, 0.0,  -1.0
2,  1.0, 0.0,  -1.0
3,  1.0, 1.0,  -1.0
...
*LOADCASE, TYPE=NONLINEARSTATIC, NAME=NODE_SURFACE_BENCHMARK
*SUPPORTS
BC
*SOLVER, DEVICE=CPU, METHOD=DIRECT
*CONSTRAINTMETHOD, TYPE=NULLSPACE
...
For this one-face benchmark each slave node has tributary area 0.25. Contact must remain inactive while the gap is positive, and the total master and slave contact resultants must be equal and opposite. The complete input deck is stored in \file{examples/10_contact/103_contact_node_surface_parallel_blocks_c3d8.inp}.
