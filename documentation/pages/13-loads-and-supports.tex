...
Loads and supports are assigned to named collectors first. A support collector is a named list of support entries. A load collector is a named list of load entries. A loadcase later selects one or more of these collectors with \cmd{SUPPORTS} and \cmd{LOADS}. Load collectors are never assembled unless they are listed in \cmd{LOADS}. Support collectors are normally selected explicitly with \cmd{SUPPORTS}; when no support collector is listed, the solver may use the special all-support collector if it exists.
