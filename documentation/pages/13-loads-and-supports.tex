...
In \val{LINEARSTATIC}, the full prescribed value is applied. In \val{NONLINEARSTATIC} load control, a nonzero prescribed value is ramped proportionally with the load factor: it is zero at \(\lambda=0\) and reaches the entered value at \(\lambda=1\). Arc-length control currently supports only homogeneous constraints and therefore rejects nonzero prescribed displacements or rotations.
...
\]
where \(t\) is the traction vector and \(N\) contains the element shape functions on the loaded surface. \cmd{PLOAD} defines a scalar pressure. A positive pressure acts into the selected surface, opposite to its normal:
\[
t = -p n .
\]
The selected surface side and its normal direction therefore determine the pressure sign convention. For shell surfaces, \val{SPOS} and \val{SNEG} select opposite normals on the same geometric midsurface.
