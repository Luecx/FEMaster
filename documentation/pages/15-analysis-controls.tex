...
After two accepted states are available, the initial reduced displacement for a new attempt is predicted by linear secant extrapolation:
\[
q_{i+1}^{(0)}
=q_i
...
(q_i-q_{i-1}).
\]
The load factor itself is not predicted; it remains the prescribed target \(\lambda_{i+1}\). The first increment starts from the accepted initial state. Rejected attempts restore the last accepted state, leave the secant history unchanged, and form a new predictor for the reduced target increment after cutback.

For inhomogeneous constraints, load control recovers the full displacement as
\[
u(\lambda)=\lambda u_p+Tq.
\]
Prescribed support values therefore ramp proportionally from zero to their entered value and are satisfied exactly at every target load factor.
...
\val{NONLINEARSTATIC} currently requires \key{METHOD=DIRECT} and \key{TYPE=NULLSPACE}. Arc-length control additionally requires homogeneous constraints and rejects nonzero prescribed displacements or rotations. Zero-row regularization changes the tangent and should be enabled only when its numerical stabilization is intended. It is not a physical stiffness contribution.
\end{warningbox}
