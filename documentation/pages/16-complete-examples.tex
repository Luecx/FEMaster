...
\section{Nonlinear axial response with tabulated J2 plasticity}

This two-node bar illustrates the same \cmd{ELASTIC}/\cmd{PLASTIC} material definition in the native and supported Abaqus parsers. The initial yield force for the 100\,mm\(^2\) section is \(A\sigma_y=25\,000\) N. The prescribed 27\,000 N tensile force is above this initial threshold; the nonlinear solution therefore requires constitutive history rather than merely using the elastic stiffness. The table gives \emph{equivalent plastic strain}, not total strain. No mass density is required for this quasistatic case.
...
Because the Abaqus deck uses geometric nonlinearity, compare results using the same load, deformation, constitutive stress measure, and nonlinear kinematics when checking against another implementation.
...
The modal example uses a two-element truss with distributed material density and requests the lowest five eigenpairs. Only axial motion is left free. It demonstrates distributed structural mass; the following complete example separately demonstrates the shared concentrated \val{MASS} element/property syntax.
...
\section{Eigenfrequency with a concentrated MASS element}

This benchmark isolates the new one-node concentrated-mass path. Two identical axial rods connect a central mass node to fixed supports on both sides. The rods carry no material density, so the only inertia is the \val{MASS} element at the centre. The centre node is guided so only \(U_x\) is free.

The example uses N, mm, and s. In this consistent unit system the mass unit is tonne, because
\[
1\ \mathrm{N}\,\mathrm{s}^2/\mathrm{mm}=1\ \mathrm{t}.
\]
Thus \(m=0.01\) represents \(10\,\mathrm{kg}\).

Each rod has
\[
k=\frac{EA}{L}
=\frac{210000\cdot1}{1000}
=210\ \mathrm{N/mm}.
\]
A positive displacement of the central node extends one rod and shortens the other, so both provide restoring stiffness and
\[
k_\mathrm{eq}=2k=420\ \mathrm{N/mm}.
\]
The exact circular eigenfrequency and frequency are therefore
\[
\omega_1=\sqrt{\frac{k_\mathrm{eq}}{m}}
=\sqrt{\frac{420}{0.01}}
\approx204.939\ \mathrm{s}^{-1},
\]
\[
f_1=\frac{\omega_1}{2\pi}
\approx32.617\ \mathrm{Hz}.
\]
...
This model is useful as a direct regression check for \cmd{ELEMENT, TYPE=MASS} and \cmd{MASS}: the stiffness comes entirely from the two trusses, the inertia comes entirely from the point element, and the expected finite eigenfrequency is approximately \(32.617\,\mathrm{Hz}\).
