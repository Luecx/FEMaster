...
When converting uniaxial test data to this table, subtract the elastic strain from the measured total strain before specifying the plastic-strain ordinate. Stress values and Young's modulus must use the same stress unit. For finite-strain applications, ensure that the stress and strain measures of the source test curve are appropriate to the chosen material calibration.
...
\begin{keyworddata}
\val{yield stress, equivalent plastic strain} & One hardening point per row; the first plastic strain is zero and subsequent strains are strictly increasing. \\
\end{keyworddata}
