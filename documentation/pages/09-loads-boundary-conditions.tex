...
\cmd{CLOAD} applies concentrated nodal forces and moments. Both FEMaster
readers accept the same data-row layouts, distinguished by the number of values
after the target:

\begin{keyworddata}
\val{target, dof, magnitude} & One-based structural DOF (1--6) and its load magnitude (Abaqus form). \\
\val{target, Fx, Fy, Fz[, Mx, My, Mz]} & Native vector form with three required force components and up to three optional moments. \\
\end{keyworddata}

The two forms may be mixed within one \cmd{CLOAD} block. In vector rows,
omitted moment components and empty entries contribute zero. A vector row with
fewer than three force components is invalid; this removes the ambiguity with
the three-token Abaqus form. DOFs 1--3 denote forces, while DOFs 4--6 denote
nodal moments. The six-vector form is a FEMaster extension when using the
Abaqus-style reader and is not portable to an unmodified Abaqus solver.

A target may be a node or a node set. The complete specified load acts on
\emph{every} node of a set, not as a resultant divided among its members.
Repeated rows contribute additively, including multiple assignments to the
same node and DOF.

\key{LOAD_COLLECTOR} is mandatory for model- or assembly-level CLOAD
definitions. Inside a native \cmd{LOADCASE} or Abaqus \cmd{STEP}, omitting it
uses an automatically selected analysis-local collector, which is included
in the active analysis without a separate \cmd{LOADS} entry. An explicitly
named collector inside an analysis is also included automatically. Outside an
analysis, an unnamed CLOAD is rejected rather than silently creating a global
default collector. Model-level named CLOAD definitions in the Abaqus-style
reader are a FEMaster extension.

Native \key{ORIENTATION} defines the local load basis. Abaqus nodal
\cmd{TRANSFORM} assignments provide each target node's load basis instead.
\key{AMPLITUDE} applies the existing reader-specific analysis and step
semantics. Non-follower real Abaqus loads are supported; \val{FOLLOWER}
and \val{IMAGINARY} remain unsupported.

\begin{keywordparameters}
\key{LOAD_COLLECTOR} & Optional within LOADCASE/STEP, required at model/assembly level. \\
\key{ORIENTATION} & Native optional named local load basis. \\
\key{AMPLITUDE} & Optional named load amplitude where supported. \\
\key{FOLLOWER} & Abaqus flag, unsupported. \\
\key{REAL}/\key{IMAGINARY} & Abaqus flags; imaginary loading is unsupported. \\
\end{keywordparameters}
