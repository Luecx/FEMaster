...
Native \cmd{VLOAD} applies a distributed three-component body-force or acceleration-type vector to an element or element set. The load is integrated with the physical mass measure of each selected structural element. For continuum and line elements this measure is obtained from density and geometry; for a regular \val{TYPE=MASS} point element it is the complete concentrated scalar assigned by \cmd{MASS}.

For mass-proportional acceleration such as gravity, the intended acceleration vector must use units consistent with the model. In a mm--s system, standard gravity is approximately \(9810\,\mathrm{mm/s^2}\). If a target ELSET contains a regular MASS element of mass \(m\), its contribution is the concentrated nodal force \(m\boldsymbol a\). Auxiliary native \cmd{POINTMASS} objects do not belong to public ELSETs and are therefore not loaded through \cmd{VLOAD}.
