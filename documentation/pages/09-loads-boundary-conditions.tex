...
\cmd{CLOAD} is a shared load keyword in the native FEMaster and supported
Abaqus readers. Each record can use one of two forms:
\begin{keyworddata}
\val{target, dof, magnitude} & Single one-based structural DOF (1--6), with its load value. \\
\val{target, Fx, Fy, Fz[, Mx, My, Mz]} & Three mandatory force components and up to three optional moment components. \\
\end{keyworddata}
The two forms may be mixed in the same \cmd{CLOAD} block. Three force
components are mandatory in a vector row so a two-value Abaqus record is
unambiguous; omitted moment components and empty vector entries contribute
zero. DOFs 1--3 specify forces and 4--6 specify moments. A vector row within
an Abaqus-style deck is a FEMaster extension and is not portable to Abaqus.

Each row applies the specified force or moment to \emph{every} node of the
target node set; it is not divided by the number of nodes. Repeated
assignments contribute additively. An explicit \key{ORIENTATION} uses the
named coordinate system. Without that keyword, either reader honors
an assigned nodal \cmd{TRANSFORM}.

The location of the command determines the collector behavior. Outside
\cmd{LOADCASE} or \cmd{STEP}, \key{LOAD_COLLECTOR} is mandatory; omitting it
is a parsing error. Within either analysis block, an unnamed \cmd{CLOAD}
belongs directly to the active analysis via an implicit private collector,
without an extra \cmd{LOADS} line. Explicitly named collectors used within
an analysis are automatically selected as well. Global named collectors
remain reusable by referring to them from \cmd{LOADS} inside a LOADCASE
or STEP (a FEMaster extension to Abaqus input).

\cmd{STEP} and its supported Abaqus-style procedure cards are also accepted
by the native reader. At most one \cmd{STEP} is currently supported per input
deck; native \cmd{LOADCASE} blocks can appear before or after it and are
executed in source order. This does not provide state transfer between analyses.
A STEP-local \cmd{CLOAD} follows the existing
procedure-dependent amplitude semantics. In the supported Abaqus reader,
explicitly named model-level CLOAD definitions and vector rows are
FEMaster extensions, not standard Abaqus input. Real non-follower
concentrated loads are supported; \val{FOLLOWER} and \val{IMAGINARY}
remain unsupported.

\begin{keywordparameters}
\key{LOAD_COLLECTOR} & Required outside LOADCASE/STEP; optional within either analysis. \\
\key{ORIENTATION} & Optional named basis (explicit FEMaster extension in an Abaqus deck). \\
\key{AMPLITUDE} & Optional named amplitude; STEP procedure rules apply. \\
\key{FOLLOWER} & Unsupported flag. \\
\key{REAL}/\key{IMAGINARY} & Real loads supported, imaginary loads unsupported. \\
\end{keywordparameters}
...
\]
Other Abaqus \cmd{DLOAD} labels are not covered by this supported form. Regular \val{TYPE=MASS} point elements are ordinary members of their ELSETs and therefore receive the concentrated gravity force \(m\boldsymbol a\) when selected by this Abaqus \val{GRAV} target. Native \cmd{POINTMASS} definitions are auxiliary post-compile objects outside ELSET topology and are not selected by \cmd{DLOAD, GRAV}.
...
\begin{keywordparameters}
\key{LOAD\_COLLECTOR} & native required & Destination load collector. \\
...
\end{femasterdeck}
\begin{abaqusdeck}
** A mass-proportional gravity load instead:
*DLOAD
...
For structural FRT shells (\val{S3}, \val{S4}, \val{S6}, and \val{S8}), each
nodal temperature describes the shell midsurface and is assumed constant through
the thickness. The element interpolates the temperature in the shell plane and
forms the initial generalized membrane strain
\[
\boldsymbol E_{\mathrm{th}} =
\alpha(T-T_{\mathrm{ref}})
\begin{bmatrix}1&1&0&0&0&0&0&0\end{bmatrix}^{T}.
\]
The complete shell-section tangent maps this initial strain to generalized
thermal resultants, including thermal bending resultants if the section has
membrane--bending coupling. Both \val{TYPE=INTEGRATED} and \val{TYPE=ABD}
sections participate when their assigned material defines
\cmd{EXPANSION} (or its legacy alias \cmd{THERMALEXPANSION}). An ABD section with a single expansion coefficient
uses an equivalent homogeneous generalized initial-strain model; it cannot
represent arbitrary ply-specific laminate thermal expansion. The shear-panel
\val{QSPT} formulation has no membrane thermal expansion contribution.

The current \cmd{TLOAD} structural formulation assembles equivalent nodal
forces for linear/reference kinematics; it does not update the section's
constitutive thermal strain, nonlinear thermal prestress tangent, or automatic
stress/section-resultant recovery. In particular, mechanical stresses recovered
from the displacement field alone must not be interpreted as thermally
corrected stresses.
