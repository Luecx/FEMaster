...
For transient analysis, amplitude sample times use the same time unit as \cmd{TIME}. If the model uses seconds, the amplitude abscissa is in seconds as well.
...
\key{SUPPORT_COLLECTOR} specifies the named collector receiving the support. A loadcase activates one or more such collectors using \cmd{SUPPORTS}. This separation is useful for models with several support cases, for example a fully fixed verification case and a second analysis with released directions.

\key{ORIENTATION} optionally specifies a local coordinate system. The prescribed components are then expressed in that basis. This is particularly useful for cylindrical supports, sliders, or boundaries whose constrained direction is not aligned with the global axes.
...
Only constrain degrees of freedom that are physically supported. Excessive support constraints can artificially stiffen the model just as severely as missing supports can make it singular.
...
The native form presents all six generalized components in one row. Abaqus uses DOF ranges, so the same constraint may require several \cmd{BOUNDARY} rows.
...
The target may be an individual node or a node set. When a node set is used, the specified vector is applied to \emph{every} node in the set. It is not divided by the number of nodes. If the intended model quantity is one resultant force or moment distributed over a surface, a reference node with a structural/distributing \cmd{COUPLING} is often the more appropriate representation.
...
Concentrated nodal loading can create local stress singularities or mesh-dependent peaks when used to represent a physically distributed load. Use surface loading or a distributing coupling when the real load is introduced over a finite area.
...
For Abaqus surface pressure and vector traction, use \cmd{DSLOAD}.
...
Traction is force per unit area. Do not enter the total desired resultant unless the loaded area is exactly one in the chosen unit system.
...
If the pressure acts in the wrong direction, first verify the surface side and element connectivity. Reversing load sign to compensate for an incorrectly oriented mesh can conceal the same orientation problem from contact and shell-result interpretation.
...
\key{CONSIDER_POINT_MASSES} controls whether native \cmd{POINTMASS} features are included in that mass distribution. Use it when concentrated equipment or lumped masses should participate in the inertial loading.
...
The spelling \cmd{INERTIALOAD} is intentional and matches the currently accepted native keyword exactly.
