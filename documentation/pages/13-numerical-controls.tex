...
\cmd{SOLVER} selects the sparse linear solution strategy for supported native loadcases and for the supported Abaqus \cmd{STEP} procedures. \key{DEVICE} may be \val{CPU} or \val{GPU}; the default is \val{CPU}. \key{METHOD} may be \val{DIRECT} or \val{INDIRECT}; the default is \val{DIRECT}.
...
\end{femasterdeck}
\begin{noabaqus}
FEMaster also accepts its own \cmd{SOLVER} extension inside an Abaqus \cmd{STEP}, \emph{after} the procedure card has created the active analysis:
\begin{syntax}
*STEP
*STATIC
...
*END STEP
\end{syntax}
This is a FEMaster extension to the supported Abaqus input subset, not a claim that every Abaqus solver-control keyword is understood.
\end{noabaqus}
\end{dialectexamples}
