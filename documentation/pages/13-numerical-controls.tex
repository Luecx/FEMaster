...
These settings can strongly affect robustness and computational cost, but they should not be used to compensate for an incorrect physical model. A singular system caused by a mechanism, missing Section, disconnected mesh, or inadequate support definition should be corrected at model level before numerical regularization is increased.
...
A direct method factorizes the sparse equation matrix and then solves by substitutions. It is generally the robust default because convergence does not depend on an iterative stopping process. The principal disadvantage is factorization memory, which can become substantial for large three-dimensional models.
...
Start with the direct CPU solver for model development and verification unless model size provides a clear reason to choose another path. Change the linear solver only after the physical model and constraints are known to be correct.
...
Nullspace projection is robust for general linear constraint equations and keeps the reduced system in structural form. It is usually the safest default when couplings, ties, and other multi-point constraints create a complex equation set.
...
Use this method only with a supported direct backend. It is particularly useful when multiplier reactions are of interest or when preserving the original displacement unknowns is advantageous.
...
For dense or strongly coupled general equations, nullspace projection may be more robust because it does not require a favourable substitution ordering.
...
Adaptive stepping does not alter the equilibrium equations. It changes only how aggressively the solution path is traversed. If a model requires extremely small increments everywhere, investigate contact transitions, material stiffness, element distortion, unstable mechanisms, or an inconsistent tangent rather than treating cutback as a complete remedy.
...
This option is intended for temporary zero-stiffness directions that can occur in nonlinear models, for example before an otherwise meaningful mechanism engages. It should not be used to conceal a genuinely unconstrained rigid-body mode or disconnected component. If increasing the regularization materially changes the physical response, the underlying model should be investigated.
...
A useful engineering check is to compare \(\Delta t\) with the shortest relevant period \(T_\mathrm{min}\). Many points per period are normally required when phase and peak amplitude matter.
...
\]
This relation is useful when choosing \(\alpha\) and \(\beta\) from desired damping ratios at two frequencies.

Rayleigh damping is convenient but frequency dependent. Coefficients fitted at two frequencies should be checked over the complete frequency band of interest, especially if the model contains modes far below or above the fitting range.
...
Frequency resolution should be chosen with the expected modal bandwidth in mind. A coarse linear sweep can miss a narrow lightly damped resonance even though the endpoints cover the correct range. Refine the frequency grid near resonances if peak amplitude or phase is important.
...
Request enough modes to cover the physical frequency or buckling-factor range of interest. The requested number is a result count, not a frequency cutoff: requesting ten modes does not guarantee that all modes below a particular frequency are captured if multiplicities or closely spaced eigenvalues are present unexpectedly.
...
\cmd{SIGMA} sets the spectral shift used by native \val{LINEARBUCKLING}. The shift guides the eigensolver toward the part of the buckling spectrum that should be extracted.
...
\val{SIGMA=0} requests the automatic shift strategy. FEMaster estimates a useful scale from the pre-buckling solution where possible and falls back to conservative stiffness/geometric-stiffness estimates if necessary. Manual shifts are mainly useful when the spectrum is already approximately known or when a particular range must be targeted deliberately.
...
No data line follows the command; \key{FIELD} names the velocity Field. Initial displacement is taken from the analysis/model state defined by the current procedure, while this command supplies the initial velocity state.
...
This is appropriate for free-flight or free-body load cases where adding an artificial fixed support would distort the structural response. It is not a replacement for real bearings, bolts, fixtures, or other physical supports.
...
This differs physically from inertia relief. \cmd{REBALANCELOADS} assumes the \emph{intended applied load itself} should be self-equilibrated and corrects the residual. \cmd{INERTIARELIEF} accepts a genuinely unbalanced free-body load and represents the rigid-body acceleration needed to balance it.

Do not rebalance a load merely to make an unsupported model solvable. If the real applied load has a physical resultant, removing it changes the load case.
