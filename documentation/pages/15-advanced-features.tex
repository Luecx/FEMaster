...
Point masses are useful for equipment, payload, actuator, engine, sensor, or other concentrated inertia that should influence eigenfrequency, transient, inertial-load, or inertia-relief calculations without explicitly meshing the attached body.

If the target set contains several nodes, the specified feature is applied at every target node. A mass value should therefore represent the mass \emph{per target node}, not automatically the total mass intended for the whole set. If a total payload mass should be distributed over several nodes, divide it deliberately or use a reference-node representation appropriate to the model.
...
Concentrated inertia can strongly affect high-frequency modes when placed far from a structure's centre of rotation. Check both mass magnitude and rotary inertia rather than modelling a large rigid payload with translational mass alone if rotational dynamics matter.
...
\rho_e^p.
\]
The command does not automatically force values into the interval \([0,1]\). If the intended optimization uses physical density fractions, the external workflow should provide values consistent with that interpretation. Negative density values or inappropriate exponents can produce nonphysical stiffness scaling.
...
Because compliance sensitivity contains factors proportional to \(1/\rho_e\), exactly zero density is generally unsuitable for gradient-based topology workflows even if a zero-stiffness element appears conceptually attractive.
...
\]
used to construct an additional element-wise material rotation. This rotation acts in addition to the Section's base material orientation. It is therefore intended for optimization or parameter studies in which the preferred anisotropic material direction varies from element to element.
...
The angle units must match FEMaster's Euler-angle convention used by this feature. When generating Fields from an external optimization program, use the same rotation order and sign convention consistently across all elements.

\begin{keywordparameters}
\key{FIELD} / \key{NAME} & required & Three-component \val{ELEMENT} Field of material-orientation angles. \\
\end{keywordparameters}
...
\]
The exact reported Field should be interpreted according to FEMaster's topology result definitions, but the sign and penalization dependence follow this standard stiffness-scaling idea.
