...
\cmd{MASS} & \checkmark & \checkmark & Advanced Model Features; isotropic point-element mass \\
...
\val{MASS} & \val{MASS} & 1-node point element carrying a separately assigned isotropic \cmd{MASS} property \\
\bottomrule
\end{longtable}
\normalsize
...
\cmd{EQUATION} and the \val{MASS} element/property pair are shared by the native and Abaqus parsers. The current \cmd{MASS} subset is isotropic translational mass only; native \cmd{POINTMASS} remains the more general NSET-based representation for rotary inertia and ground-spring terms. Regular \val{MASS} elements remain ordinary ELSET members and therefore participate in element-targeted mass-proportional loads and regular inertia-relief mass properties.
