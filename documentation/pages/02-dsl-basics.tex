...
\section{Dependency-ordered parsing}

FEMaster processes an input deck in dependency-ordered stages. Topology is created before fields whose sizes depend on element-local enumeration. Enumeration-dependent fields such as \val{ELEMENT_NODAL}, \val{ELEMENT_IP}, and \val{ELEMENT_MP} are therefore available before later model construction uses them.

Commands that reference such fields should still appear after the corresponding \cmd{FIELD} definition in the input deck. In particular, a shell-director field must be defined before \cmd{NORMAL, FIELD=<name>} selects it. The repeated parser passes are an implementation detail; users should write decks in logical dependency order rather than relying on a later command to define data referenced earlier in the file.
