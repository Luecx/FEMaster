...
The complete formulation reference is given in Chapter~\ref{chap:elements}. That chapter documents the supported solid, shell, beam, truss, and point elements individually, including node ordering, numerical behaviour, applicability, and the existing element schematics.

The native and Abaqus input names are not always identical. The solid labels \val{C3D4}, \val{C3D5}, \val{C3D6}, \val{C3D8}, \val{C3D8R}, \val{C3D10}, \val{C3D15}, \val{C3D20}, and \val{C3D20R} are shared. Native FEMaster accepts both \val{T3} and the Abaqus spelling \val{T3D2} for the two-node truss. Abaqus shell labels \val{S3/S3R}, \val{S4/S4R}, \val{S6/S6R}, and \val{S8/S8R} are read into the corresponding finite-rotation shell formulations. Native FEMaster accepts the short shell labels \val{S3}, \val{S4}, \val{MITC4}, \val{S6}, \val{S8}, and \val{MITC8} as aliases of the corresponding current finite-rotation FRT shell implementations; these labels no longer select separate legacy linear shells. The FRT names and \val{QSPT} are also accepted directly. The one-node \val{MASS}, \val{ROTARYI}, and \val{SPRING1} labels are shared by both input dialects.

\val{TYPE=MASS}, \val{TYPE=ROTARYI}, and \val{TYPE=SPRING1} define one-node point elements. Each preserves its own element ID, one-node connectivity, ELSET membership, and Part/Instance expansion. Internally the three labels use the same point-element topology; the associated property keyword supplies the mechanical contribution. \cmd{MASS} assigns translational mass, \cmd{ROTARY INERTIA} assigns diagonal rotary inertia, and \cmd{SPRING} assigns one constant linear point-to-ground stiffness. The formulations and property syntax are documented in Chapter~\ref{chap:elements} and Chapter~\ref{chap:sections}.

An element definition provides topology and kinematics, but not the complete physical properties. Solids, shells, beams, and trusses require the corresponding Section definitions to obtain their intended material and geometric stiffness. Likewise, regular point elements remain mechanically inactive until the corresponding point property is assigned; Chapter~\ref{chap:sections} documents these assignments.
...
\val{n1, n2, ...} & Node connectivity in the formulation-specific order. \val{MASS}, \val{ROTARYI}, and \val{SPRING1} use exactly one node. \\
\end{keyworddata}
...
100, 20
*ELEMENT, TYPE=ROTARYI, ELSET=PAYLOAD_ROTARY
101, 20
*ELEMENT, TYPE=SPRING1, ELSET=PAYLOAD_SPRING
102, 20
