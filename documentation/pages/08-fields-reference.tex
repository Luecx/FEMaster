...
\cmd{FIELD} creates and optionally populates a named Field. It is a native FEMaster command; the supported Abaqus input format currently has no generic equivalent. The command may be used at model level and, where appropriate, in a native \cmd{LOADCASE}.

\key{NAME} identifies the Field. \key{TYPE} selects its domain, \key{COLS} specifies the number of numerical components per location, and \key{FILL} defines how entries are initialized before explicit data rows are applied. \val{FILL=ZERO} is the default. \val{FILL=NAN} initializes unspecified components with NaN and is useful when missing data must remain distinguishable from a physical zero.
...
Field values may be ordinary floating-point numbers and may also use \val{NAN}, \val{+NAN}, \val{-NAN}, \val{INF}, \val{+INF}, and \val{-INF}. Non-finite values are mainly useful for partially prescribed or diagnostic Fields; they should only be used when the feature consuming the Field is defined to handle them.
...
Do not confuse user IDs with element-local indices. User node and element IDs may include \val{0} and may be sparse. Element-local node, integration-point, and material-point indices are always positional indices beginning at \val{0}.
...
This mechanism is useful when shell orientation cannot be described adequately by a single normal per geometric node. Because the data are element-nodal, two neighbouring shell elements may intentionally prescribe different director vectors at the same geometric node. This is useful at folds, discontinuous shell normals, imported midsurfaces, or other locations where averaging a single nodal direction would be inappropriate.

The vectors supplied in the Field are used as prescribed shell-normal information. Missing entries may be left as NaN when the shell-normal workflow is intended to complete them from the element geometry. Users should nevertheless inspect the resulting shell orientations carefully: normal direction influences shell top/bottom interpretation, pressure direction, local section directions, and the sign of bending-related result quantities.
...
\end{femasterdeck}
\begin{noabaqus}
The supported Abaqus reader does not expose FEMaster's generic Field-based \cmd{NORMAL} selector. Use the Abaqus syntax that is explicitly supported for the relevant model definition, or use native FEMaster input when element-nodal directors must be prescribed directly.
\end{noabaqus}
\end{dialectexamples}

\notesheading

The local node index is the position inside the element connectivity and is zero based. It is not the connected node's user ID. This distinction is what allows two shell elements sharing one geometric node to store different element-nodal directors at that location.
