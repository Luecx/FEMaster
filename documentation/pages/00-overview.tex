...
Neither syntax forces the user to introduce Parts and Instances for a simple model. In native FEMaster input, nodes and elements can be defined directly. The structured Part/Assembly representation becomes useful when components are reused, when several copies of one component are placed in different positions, when independent local ID spaces improve readability, or when an Abaqus-style assembly is being used.

\begin{notebox}[Optional Part/Assembly hierarchy]
A native FEMaster model does not require \cmd{PART}, \cmd{ASSEMBLY}, or \cmd{INSTANCE}. A flat model is a fully supported workflow. Use the hierarchy when it makes the physical organization of the model clearer.
\end{notebox}
...
\begin{enumerate}
  ...
  \item inspect the resulting displacement, force, stress, strain, mode-shape, buckling, transient, or harmonic results.
\end{enumerate}

This order is also a useful way to diagnose an incorrect model. If a calculation fails or gives an unexpected result, first verify geometry and element connectivity, then Section and material assignments, then region definitions and coordinate directions, then supports and constraints, and finally the numerical analysis settings.
...
Do not confuse this nodal six-component order with the six-component stress/strain order. Stress and strain tensors use the engineering component convention documented in the result chapter.
