...
FEMaster is a finite-element solver for mechanical analyses. The core workflows are linear, with an incremental nonlinear static path for large-displacement/contact-style problems. Models are described with an input-deck language in which commands start with an asterisk, command options are written as keywords, and numerical data follows on subsequent lines.
...
\val{NONLINEARSTATIC} & Incremental nonlinear static analysis with updated coordinates, tangent stiffness iterations, and optional penalty contact. \\
