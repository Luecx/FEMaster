...
\key{--no-res} & Disables native FEMaster \file{.res} result output, leaving \file{.frd} output enabled by default. \\
\key{--no-frd} & Disables CalculiX/CGX \file{.frd} result output, leaving native \file{.res} output enabled by default. \\
\bottomrule
\end{longtable}

By default both result writers are enabled. Combining \key{--no-res} and \key{--no-frd} leaves parsing, solution, and console diagnostics enabled without writing either result format.
