...
FEMR output adds an LZ4 build dependency. The current CMake configuration requires the LZ4 headers and library even if only \file{.res} and \file{.frd} output will be selected at runtime. Install the LZ4 development package (for example, \file{liblz4-dev} on Debian/Ubuntu), or supply \key{LZ4\_INCLUDE\_DIR} and \key{LZ4\_LIBRARY} to CMake if its package cannot be found.
...
cmake --build --preset macos-accelerate
\end{shellcmd}

The Linux presets cover CPU-only builds, OpenMP builds, oneMKL builds, CUDA builds, cuDSS builds, and combined CUDA/oneMKL configurations. Windows presets are split between MSVC and IntelLLVM toolchains. macOS provides CPU, OpenMP, Apple Accelerate, and combined OpenMP/Accelerate presets. The current preset names are \val{macos}, \val{macos-omp}, \val{macos-accelerate}, and \val{macos-omp-accelerate}. \file{CMakePresets.json} contains the exact preset list for the checked-out version.

Apple Accelerate is available only on macOS. When enabled, supported symmetric CPU-direct sparse systems use the Accelerate LDLT backend. General nonsymmetric CPU-direct systems continue to use Eigen SparseLU. Accelerate and oneMKL are mutually exclusive build options.
...
\key{FEMASTER_ENABLE_CUDSS} & Enables NVIDIA cuDSS for CUDA sparse direct solves. Requires CUDA. \\
\key{FEMASTER_ENABLE_MKL} & Enables Intel oneMKL for Eigen operations and PARDISO. \\
\key{FEMASTER_ENABLE_ACCELERATE} & Enables Apple Accelerate for supported sparse direct solves on macOS. Mutually exclusive with oneMKL. \\
\key{FEMASTER_MKL_SEQUENTIAL} & Selects the sequential oneMKL threading layer. \\
...
\key{FEMASTER_DOUBLE_PRECISION} & Enables double-precision arithmetic. \\
\key{FEMASTER_FAST_EIGEN_COMPILE} & Enables Eigen compile-time reduction flags. \\
...
FEMaster model.inp --format abaqus
FEMaster model.inp --output-format res
FEMaster model.inp --output-format frd femr
\end{shellcmd}
...
\key{--output FILE} & Explicit output base or result file path. The suffix \file{.res}, \file{.frd}, \file{.femr}, or \file{.inp} is stripped before writers are opened, so \file{--output run.res} produces \file{run.res} and \file{run.frd}. \\
\key{--ncpus N} & Number of threads allowed for parallel assembly and compute sections. \\
\key{--format femaster|abaqus} & Select the native or supported Abaqus input parser; the default is \val{femaster}. This option is independent of the output formats. \\
\key{--output-format} & Selects one or more result formats from \key{res}, \key{frd}, and \key{femr}. \\
\bottomrule
\end{longtable}

By default the \file{.res} and \file{.frd} writers are enabled. Supplying
\key{--output-format} replaces that default selection. FEMR version 1 uses LZ4
compression for independently loadable field chunks.
...
\key{--doc-width N} & Set the wrapping width for documentation output; \val{0} disables wrapping. \\
