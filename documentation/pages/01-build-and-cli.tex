...
\key{--no-res} & Disable native FEMaster \file{.res} result output. This is useful when only CalculiX/CGX \file{.frd} output is needed. \\
\key{--no-frd} & Disable CalculiX/CGX \file{.frd} result output. This is useful when only the native FEMaster \file{.res} output is needed. \\
\bottomrule
\end{longtable}

By default both result writers are enabled. \key{--no-res} and \key{--no-frd} can be combined if a run should only parse, solve, and print console diagnostics without writing either result format.
