...
\cmd{HYPERELASTIC} & Assign Abaqus-style Neo-Hooke hyperelastic behaviour. \\
...
\cmd{SHELLSECTION} & Assign integrated or prescribed ABD section behaviour to shell elements. \\
...
\cmd{FIELD} & Define node, element, element-nodal, integration-point, or material-point data fields. \\
\cmd{NORMAL} & Select a three-component element-nodal field for shell directors. \\
...
\cmd{CONTACT} & Define frictionless dual-mortar surface-to-surface contact for nonlinear static analysis. \\
\bottomrule
\end{longtable}
...
\cmd{MODEL} & Optionally start an explicit top-level model block. \\
\cmd{END} & Optionally mark the end of a loadcase. \\
...
\cmd{CONSTRAINTMETHOD} & Select Nullspace, Lagrange, or elimination constraint handling. \\
\cmd{NONLINEAR} & Configure path control, increment adaptation, Newton convergence, and tangent regularization for nonlinear static analysis. \\
...
\cmd{DAMPING} & Set Rayleigh damping for transient or harmonic analysis. \\
\cmd{FREQUENCIES} & Define the excitation-frequency sweep for linear harmonic analysis. \\
