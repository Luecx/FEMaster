...
For \val{LINEARHARMONIC}, the native writer currently writes the real/imaginary field blocks in frequency-sweep order but does not encode the supplied frame value in the \file{.res} field header. The associated frequency is therefore determined from the \cmd{FREQUENCIES} sequence and the block order.
