...
\val{MASS} & 1 & Zero-dimensional point element with separately assigned translational mass. \\
\val{ROTARYI} & 1 & Zero-dimensional point element with separately assigned rotary inertia. \\
\val{SPRING1} & 1 & Zero-dimensional point-to-ground spring acting in one generalized nodal direction. \\
\bottomrule
\end{longtable}

The Abaqus reader accepts the same solid names \val{C3D4}, \val{C3D5}, \val{C3D6}, \val{C3D8}, \val{C3D8R}, \val{C3D10}, \val{C3D15}, \val{C3D20}, and \val{C3D20R}. Abaqus \val{T3D2} is mapped to FEMaster \val{T3}. Abaqus \val{B33} is mapped to FEMaster \val{B33}. The one-node \val{MASS}, \val{ROTARYI}, and \val{SPRING1} types are shared directly by both input dialects and map to the same internal point-element implementation. Abaqus shell names are translated as shown below.
...
Point elements are zero-dimensional structural elements attached to a single node. They preserve element identity and ELSET membership while their physical contribution is supplied by an assigned point-mass section rather than by continuum geometry or a material law. The native FEMaster and supported Abaqus readers expose three regular one-node topologies: \val{MASS}, \val{ROTARYI}, and \val{SPRING1}. Internally all three use the same \texttt{PointElement}; the corresponding property keyword determines which generalized diagonal coefficient is populated.

A point element without an assigned point property is intentionally inert. It has zero stiffness, zero mass, no stress or strain state, and activates no degrees of freedom. Property definitions are documented in Chapter~\ref{chap:sections}; this section focuses on the element topology and kinematic role.
...
\val{MASS} is a one-node point element accepted by both input dialects. The \cmd{ELEMENT} record creates only topology: an element ID, one connected node, and normal ELSET membership. The scalar physical mass is assigned separately by \cmd{MASS, ELSET=...}.
...
\clearpage
\subsection{\val{ROTARYI}: 1-node rotary-inertia element}
...
\clearpage
\subsection{\val{SPRING1}: 1-node point-to-ground spring}
...
Native \cmd{POINTMASS} provides an alternative NSET-based convenience syntax when mass, rotary inertia, and several diagonal translational or rotational springs should be combined directly on the same target nodes. It creates auxiliary post-compile point elements rather than regular ELSET members.
...
\begin{itemize}
  ...
  \item \val{MASS}, \val{ROTARYI}, and \val{SPRING1} provide concentrated diagonal point inertia or ground stiffness without continuum geometry or stress recovery;
