...
\val{MASS} & 1 & Zero-dimensional point element with separately assigned concentrated mass. \\
\bottomrule
\end{longtable}

The Abaqus reader accepts the same solid names \val{C3D4}, \val{C3D5}, \val{C3D6}, \val{C3D8}, \val{C3D8R}, \val{C3D10}, \val{C3D15}, \val{C3D20}, and \val{C3D20R}. Abaqus \val{T3D2} is mapped to FEMaster \val{T3}. Abaqus \val{B33} is mapped to FEMaster \val{B33}. The one-node \val{MASS} type is shared directly by both input dialects. Abaqus shell names are translated as shown below.
...
\clearpage
\subsection{\val{C3D4}: 4-node tetrahedral solid}
...
\clearpage
\subsection{\val{C3D5}: 5-node pyramid input}
...
\clearpage
\subsection{\val{C3D6}: 6-node wedge solid}
...
\clearpage
\subsection{\val{C3D8}: 8-node hexahedral solid}
...
\clearpage
\subsection{\val{C3D8R}: reduced-integration 8-node hexahedral solid}
...
\clearpage
\subsection{\val{C3D10}: 10-node quadratic tetrahedral solid}
...
\clearpage
\subsection{\val{C3D15}: 15-node quadratic wedge solid}
...
\clearpage
\subsection{\val{C3D20}: 20-node quadratic hexahedral solid}
...
\clearpage
\subsection{\val{C3D20R}: 20-node reduced-integration hexahedral solid}
...
\clearpage
\subsection{\val{S3}: legacy 3-node triangular shell}
...
\clearpage
\subsection{\val{MITC3FRT}: finite-rotation 3-node shell}
...
\clearpage
\subsection{\val{S4}: legacy 4-node quadrilateral shell}
...
\clearpage
\subsection{\val{MITC4}: legacy 4-node shell with tied shear interpolation}
...
\clearpage
\subsection{\val{MITC4FRT}: finite-rotation 4-node MITC shell}
...
\clearpage
\subsection{\val{S6}: legacy 6-node triangular shell}
...
\clearpage
\subsection{\val{MITC6FRT}: finite-rotation 6-node shell}
...
\clearpage
\subsection{\val{S8}: legacy 8-node quadrilateral shell}
...
\clearpage
\subsection{\val{MITC8}: legacy 8-node MITC shell}
...
\clearpage
\subsection{\val{MITC8FRT}: finite-rotation 8-node MITC shell}
...
\section{Specialized panel elements}

\val{QSPT} is retained as a distinct panel idealization rather than grouping it with the general shell family, because it does not provide the complete membrane/bending/shear shell response documented above.

\clearpage
\subsection{\val{QSPT}: quadrilateral shear-panel element}
...
\section{Point elements}

Point elements are zero-dimensional structural elements attached to a single node. They preserve element identity and ELSET membership while their physical contribution is supplied by an assigned point-mass section rather than by continuum geometry or a material law.

\clearpage
\subsection{\val{MASS}: 1-node concentrated-mass element}
...
\val{MASS} is a one-node point element accepted by both the native FEMaster and Abaqus readers. The \cmd{ELEMENT} record creates only topology: an element ID, one connected node, and normal ELSET membership. The scalar physical mass is assigned separately by \cmd{MASS, ELSET=...}. This separation allows point elements to participate in Parts, Instances, assembly-level sets, output numbering, and element-targeted loading like other regular elements.

A point element without an assigned point-mass property is intentionally inert. It has zero stiffness, zero mass, no stress or strain state, and activates no degrees of freedom. Once the supported isotropic \cmd{MASS} property is assigned, its element mass matrix is
\[
M_e=\operatorname{diag}(m,m,m,0,0,0),
\]
so the three translational components at the connected node are active. The supported input mapping does not assign rotary inertia or ground-spring stiffness to \val{MASS} elements.

The mass value belongs to each element in the target ELSET. Several \val{MASS} elements may therefore reference the same node; each represents an independent concentrated contribution and the assembled mass is additive. Conversely, an ELSET used by \cmd{MASS} must contain only point elements; mixed structural/point sets are rejected before assignment.

The point element has zero geometric volume, but its density-scaled field integration interprets \(m\) as the complete concentrated mass measure. Consequently, a regular \val{MASS} element contributes to the global mass matrix, eigenfrequency and transient analyses, inertia relief, and element-targeted mass-proportional body or inertial loads. For example, a native \cmd{VLOAD} or supported Abaqus \cmd{DLOAD} with \val{GRAV} produces the concentrated force \(m\boldsymbol a\) when the selected element region contains the point element.

Part-local \val{MASS} elements and their \cmd{MASS} property assignments are copied to every Instance during model compilation. An assembly-level \cmd{MASS} assignment instead targets compiled point elements through an assembly ELSET. This makes the same one-node topology usable in flat, Part/Instance, and assembly-selected models.

Native \cmd{POINTMASS} provides a second user-facing way to define concentrated properties directly on an NSET. It is more general because it additionally supports rotary inertia and diagonal translational/rotational ground stiffness. Internally both paths use the same point-element/point-mass-section mechanics, but native \cmd{POINTMASS} creates auxiliary post-compile point elements that are not public ELSET members; the regular \val{MASS} topology documented here remains an ordinary element in the compiled model.
...
\clearpage
\subsection{\val{B33}: 3D beam element}
...
\clearpage
\subsection{\val{T3}: 2-node truss element}
...
\begin{itemize}
  ...
  \item \val{MASS} provides concentrated translational inertia without continuum geometry or stress recovery;
