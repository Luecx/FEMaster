...
\val{S3} & 3 & three-node triangular finite-rotation shell; alias of \val{MITC3FRT}. \\
\val{S4} & 4 & four-node quadrilateral finite-rotation shell; alias of \val{MITC4FRT}. \\
\val{MITC4} & 4 & four-node quadrilateral finite-rotation shell; alias of \val{MITC4FRT}. \\
\val{S6} & 6 & six-node triangular finite-rotation shell; alias of \val{MITC6FRT}. \\
\val{S8} & 8 & eight-node quadrilateral finite-rotation shell; alias of \val{MITC8FRT}. \\
\val{MITC8} & 8 & eight-node quadrilateral finite-rotation shell; alias of \val{MITC8FRT}. \\
...
The native and Abaqus shell labels above select the same finite-rotation implementations. In particular, native \val{S4}, \val{MITC4}, and \val{MITC4FRT}, as well as Abaqus \val{S4}/\val{S4R}, all select the current four-node \texttt{FRTShellS4}. The former separate linear shell implementations have been removed; keeping their input labels does not retain their former numerical behavior.
...
\begin{warningbox}[Shell input labels and current formulations]
The native labels \val{S3}, \val{S4}, \val{MITC4}, \val{S6}, \val{S8}, and \val{MITC8} remain accepted as \emph{aliases} for the current finite-rotation \val{MITC3FRT}, \val{MITC4FRT}, \val{MITC6FRT}, and \val{MITC8FRT} implementations. They do not select the removed legacy linear shell formulations. The supported Abaqus shell labels also map to these four FRT implementations.
\end{warningbox}

\clearpage
\subsection{\val{S3}: 3-node finite-rotation triangular shell (alias)}
...
\val{S3} is an accepted native input label for \val{MITC3FRT}. Triangular shell meshes cover irregular midsurfaces. This spelling selects the same current finite-rotation implementation as \val{MITC3FRT}; it is not a retained linear shell formulation.
...
\clearpage
\subsection{\val{S4}: 4-node finite-rotation quadrilateral shell (alias)}
...
\val{S4} is an accepted native input label for \val{MITC4FRT}. This spelling selects the same current finite-rotation MITC quadrilateral implementation as \val{MITC4FRT}. It does not select the removed low-order linear shell implementation.

\clearpage
\subsection{\val{MITC4}: 4-node finite-rotation MITC shell (alias)}
...
\val{MITC4} is an accepted native input label for \val{MITC4FRT}. This spelling is an alias for \val{MITC4FRT}, including its finite-rotation kinematics and MITC shear treatment; it does not select a separate legacy linear formulation.
...
\clearpage
\subsection{\val{S6}: 6-node finite-rotation triangular shell (alias)}
...
\val{S6} is an accepted native input label for \val{MITC6FRT}. This spelling selects the same current finite-rotation six-node triangular implementation as \val{MITC6FRT}. Midside-node placement and curved-element distortion still affect the solution.
...
\clearpage
\subsection{\val{S8}: 8-node finite-rotation quadrilateral shell (alias)}
...
\val{S8} is an accepted native input label for \val{MITC8FRT}. This spelling selects the same current finite-rotation eight-node quadrilateral implementation as \val{MITC8FRT}. Its quadratic interpolation helps resolve smooth curvature but remains sensitive to warping and midside-node positioning.

\clearpage
\subsection{\val{MITC8}: 8-node finite-rotation MITC shell (alias)}
...
\val{MITC8} is an accepted native input label for \val{MITC8FRT}. This spelling is an alias for \val{MITC8FRT}; it no longer selects a separate legacy linear MITC shell. The current implementation uses finite rotations and MITC shear treatment.
...
The point element has zero geometric volume, but its density-scaled field integration interprets \(m\) as the complete concentrated mass measure. Consequently, a regular \val{MASS} element contributes to the global mass matrix, eigenfrequency and transient analyses, inertia relief, and element-targeted mass-proportional body or inertial loads. The supported Abaqus \cmd{DLOAD} with \val{GRAV} produces the concentrated force \(m\boldsymbol a\) when the selected element region contains the point element. Native \cmd{VLOAD} instead denotes force density and does not load this zero-volume point element.
...
\begin{itemize}
  ...
  \item all accepted shell labels select the current finite-rotation formulations; non-FRT native labels are compatibility aliases rather than separate linear elements;
