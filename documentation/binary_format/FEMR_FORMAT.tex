...
\begin{description}
  ...
  stores an explicit \texttt{int32} identifier and three \texttt{float64}
  coordinates. Each element stores its explicit identifier, a length-prefixed
  UTF-8 type name, node count and explicit node identifiers.
...
\texttt{ELEMENT} (2). Writers and readers reject all other domain codes.
...
they must not be reconstructed from element connectivity.
