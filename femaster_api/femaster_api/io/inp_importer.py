"""Importer for FEMaster/Abaqus-like keyword input decks."""
...
class InpImporter:
    """Import FEMaster keyword decks into the public Project object model."""

    def import_file(self, path: str | Path) -> Project:
        """Import one UTF-8 input file."""

        return self.import_text(Path(path).read_text(encoding="utf-8"))

    def import_text(self, text: str) -> Project:
        ...
        for item in parsed:
            ...
            # -----------------------------------------------------------------
            # Analysis steps and child commands
            # -----------------------------------------------------------------
            ...
            if name == "ELEMENT":
                ...
                for row in item.data:
                    ...
                    if len(values) < 2:
                        raise ValueError(
                            f"ELEMENT at line {item.line} requires id and connectivity"
                        )
            ...
            if name == "PROFILE":
                ...
                project.profiles.add(
                    Profile(self._required(item, "NAME"), *values[:9])
                )
                continue
            ...
            if name == "SUPPORT":
                collector = self._support_collector(
                    project,
                    self._required(item, "SUPPORT_COLLECTOR"),
                )
            ...
            if name == "RBM":
                project.constraints.add(
                    RigidBodyConstraint(self._required(item, "ELSET"))
                )
                continue
            ...
            if name == "COUPLING":
                ...
                if not slave:
                    raise ValueError(
                        f"COUPLING at line {item.line} requires SLAVE or SFSET"
                    )

                dofs = (
                    [int(value) for value in item.data[0]]
                    if item.data
                    else (1, 1, 1, 1, 1, 1)
                )
                project.constraints.add(
                    Coupling(
                        ...
                        slave,
                        type=CouplingType[
                            (item.key("TYPE", "KINEMATIC") or "KINEMATIC").upper()
                        ],
                        dofs=dofs,
            ...
            if name == "TIE":
                project.constraints.add(
                    Tie(
                        ...
                        distance=(
                            float(item.key("DISTANCE"))
                            if item.key("DISTANCE") is not None
                            else None
                        ),
                    )
                )
    ...
    def _read_part_section(
        ...
        if section_type == "ABD":
            ...
            if len(values) != 40:
                raise ValueError(
                    f"SHELLSECTION TYPE=ABD at line {item.line} requires 40 values"
                )
            sections.add(
                ABDShellSection(
                    section_name,
        ...
        thickness = (
            thickness_values[0]
            if thickness_values
            else float(item.key("THICKNESS", "1.0") or 1.0)
        )
        sections.add(
            ShellSection(
                section_name,
    ...
    def _read_model_field(self, item: KeywordBlock) -> Field:
        ...
        domain_aliases = {
            "ELEMENTNODAL": "ELEMENT_NODAL",
            "ELEMENTIP": "ELEMENT_IP",
            "IP": "ELEMENT_IP",
            "ELEMENTMP": "ELEMENT_MP",
            "MP": "ELEMENT_MP",
        }
        domain_name = domain_aliases.get(domain_name, domain_name)
        ...
        for row in item.data:
            if len(row) < index_cols:
                continue
            indices = tuple(
                self._token(value) for value in row[:index_cols]
            )
    ...
    def _read_load(self, project: Project, item: KeywordBlock) -> None:
        collector = self._load_collector(
            project,
            self._required(item, "LOAD_COLLECTOR"),
        )
        ...
        collector.add(
            InertialLoad(
                ...
                consider_point_masses=(
                    item.key("CONSIDER_POINT_MASSES", "0") == "1"
                ),
            )
        )
    ...
    def _read_step_subcommand(self, step, item: KeywordBlock) -> bool:
        ...
        if name == "SUPPORTS":
            step.supports = tuple(
                value for row in item.data for value in row if value
            )
            return True
        if name == "LOADS":
            step.loads = tuple(
        ...
        if name == "SOLVER":
            step.solver = SolverControl(
                SolverDevice[
                    (item.key("DEVICE", "CPU") or "CPU").upper()
                ],
                SolverMethod[
                    (item.key("METHOD", "DIRECT") or "DIRECT").upper()
                ],
            )
            return True
        if name == "CONSTRAINTMETHOD":
            step.constraint_method = ConstraintMethod[
                self._required(item, "TYPE").upper()
            ]
            return True
        ...
        if name == "NONLINEAR" and isinstance(step, NonlinearStaticStep):
            step.control = (
                item.key("CONTROL", step.control) or step.control
            ).upper()
            step.increments = self._int_key(item, "INCREMENTS")
            step.max_increments = self._int_key(item, "MAX_INCREMENTS")
            step.initial_increment = self._float_key(
                item,
                "INITIAL_INCREMENT",
            )
            step.minimum_increment = self._float_key(
                item,
                "MINIMUM_INCREMENT",
            )
            step.maximum_increment = self._float_key(
                item,
                "MAXIMUM_INCREMENT",
            )
