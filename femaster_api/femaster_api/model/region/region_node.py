"""Concrete node region.

Region of nodes exported through the native ``*NSET`` keyword.  The class contains only domain-specific keyword metadata; member
storage, deterministic ordering and row export are implemented by ``Region``.
Keeping each region domain in its own module makes imports and future
domain-specific validation explicit.
"""

from __future__ import annotations

from .region import Region


class NodeRegion(Region):
    """Region of nodes exported through the native ``*NSET`` keyword."""

    keyword_name = 'NSET'
    name_key = 'NAME'
