...
def validate_model(model) -> Diagnostics:
    ...
    for section in model.sections.all():
        ...
        if isinstance(section, BeamSection) and not model.profiles.has(section.profile):
    ...
    checked_loads: set[int] = set()
    for load in model.loads.all():
        checked_loads.add(id(load))
        _check_load(model, diagnostics, load)
    for collector in model.load_collectors.all():
        for load in collector.loads:
            if id(load) in checked_loads:
                continue
            checked_loads.add(id(load))
            _check_load(model, diagnostics, load)

    checked_supports: set[int] = set()
    for support in model.supports.all():
        checked_supports.add(id(support))
        _check_support(model, diagnostics, support)
    for collector in model.support_collectors.all():
        for support in collector.supports:
            if id(support) in checked_supports:
                continue
            checked_supports.add(id(support))
            _check_support(model, diagnostics, support)
...
def _check_support(model, diagnostics: Diagnostics, support) -> None:
