...
NonlinearStateManager::NonlinearStateManager(model::Model& model)
    : model_(model),
      previous_material_state_old_(model._data->material_state_old),
      previous_material_state_new_(model._data->material_state_new),
    ...
    if (state_size == 0) {
        model_._data->material_state_old = nullptr;
        model_._data->material_state_new = nullptr;
...
NonlinearStateManager::~NonlinearStateManager() {
    model_._data->material_state_old = previous_material_state_old_;
    model_._data->material_state_new = previous_material_state_new_;
    model_._data->material_state     = previous_material_state_;
}

void NonlinearStateManager::reset_material_state() {
    // Explicit old/new constitutive states make rollback copy-free. Every
    // update reads committed state and overwrites its complete trial row.
    bind_material_state();
}
...
void NonlinearStateManager::begin_contact_update_trial() {
    for (const auto& contact : model_._data->contacts) contact.begin_trial();
}
...
void NonlinearStateManager::commit_contact_trial() {
    for (const auto& contact : model_._data->contacts) contact.commit_trial();
}

void NonlinearStateManager::rollback_contact_trial() {
    for (const auto& contact : model_._data->contacts) contact.rollback_trial();
}
...
void NonlinearStateManager::bind_material_state() {
    ...
    trial_material_state_->name     = "MATERIAL_STATE_TRIAL";
    model_._data->material_state_old = committed_material_state_;
    model_._data->material_state_new = trial_material_state_;
