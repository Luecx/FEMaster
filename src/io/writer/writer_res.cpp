...
void ResWriter::close() {
    if (file_path.is_open()) file_path.close();
}
...
void ResWriter::write_field(const model::Field& field,
    ...
    if (field.domain == model::FieldDomain::ELEMENT_NODAL) {
        ...
        write_element_location_field(
            file_path, field, field_name, *model_data,
            *model_data->element_nodal_offsets, "element nodal offsets");
        return;
    }
    ...
    if (field.domain == model::FieldDomain::ELEMENT_IP) {
        ...
        write_element_location_field(
            file_path, field, field_name, *model_data,
            *model_data->element_ip_offsets, "element IP offsets");
        return;
    }
    ...
    if (field.domain == model::FieldDomain::ELEMENT_MP) {
        ...
        write_element_location_field(
            file_path, field, field_name, *model_data,
            *model_data->element_mp_offsets, "element MP offsets");
        return;
    }
