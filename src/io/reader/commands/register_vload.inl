...
inline void register_vload(fem::io::dsl::Registry& registry, model::Model& model) {
    registry.command("VLOAD", [&](fem::io::dsl::Command& command) {
                ...
                .bind([&model, orientation, amplitude](const std::string& target,
                    ...
                    if (!orientation->empty()) {
                        logging::error(model._data->coordinate_systems.has(*orientation),
                            "VLOAD: coordinate system ", *orientation, " does not exist");
                    ...
                    if (!amplitude->empty()) {
                        logging::error(model._data->amplitudes.has(*amplitude),
                            "VLOAD: amplitude ", *amplitude, " does not exist");
