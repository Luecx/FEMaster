...
void Model::add_connector(const std::string& set1,
    ...
    _connectors.emplace_back(id1, id2, _data->coordinate_systems.get(coordinate_system), type);
}
...
void Model::add_coupling(const std::string &master_set, const std::string &slave_set, Dofs coupled_dofs, constraint::CouplingType type) {
    ...
    _couplings.emplace_back(master_node, _data->node_sets.get(slave_set), coupled_dofs, type);
}
...
void Model::add_tie(const std::string& master_set, const std::string& slave_set, Precision distance, bool adjust) {
    ...
    _ties.emplace_back(master_ptr, slave_ptr, distance, adjust);
}
...
void Model::add_vload(const ID id, Vec3 load) {
    ...
    if (!_data->elements[id]->is_type(StructuralType)) return;

    auto sel = _data->elements[id]->as<StructuralElement>();
...
void Model::add_tload(std::string& temp_field, Precision ref_temp) {
    ...
    for (ElementPtr& elem : _data->elements) {
        if (elem == nullptr || !elem->is_type(StructuralType)) continue;
