 ...
 *
 * The two transverse-shear tensor fields use four boundary sampling tensors
 * and one internal tensor. The fifth scalar coefficient is the mean of the two
 * associated interior sampling values, while its physical tensor basis is
 * evaluated at the element center. Every assumed-strain interpolation is
 ...
 *
 * The `rt` and `st` transverse-shear contributions are reconstructed as
 * complete physical sampling tensors. Boundary values retain the physical
 * tangent and normal basis of their own sampling point. The fifth scalar
 * coefficients are the means at the RA/RB and SA/SB pairs, while both fifth
 * tensor bases are evaluated at the element center. The resulting physical
 * shear tensor is projected into the target natural components before the
 * common shell kernel performs its pointwise natural-to-local transformation.
 *
...
void FRTShellS8::apply_mitc_natural(
    ...
    const Precision r_a = point.r / a;
    const Precision s_a = point.s / a;

    const Precision h_rt_5 =
        (Precision(1) - r_a * r_a)
        * (Precision(1) - point.s * point.s);

    const Precision h_st_5 =
        (Precision(1) - s_a * s_a)
        * (Precision(1) - point.r * point.r);

    const Precision h_rt_1 = Precision(0.25) * (Precision(1) + r_a)
                           * (Precision(1) + point.s) - Precision(0.25) * h_rt_5;
    const Precision h_rt_2 = Precision(0.25) * (Precision(1) - r_a)
                           * (Precision(1) + point.s) - Precision(0.25) * h_rt_5;
    const Precision h_rt_3 = Precision(0.25) * (Precision(1) - r_a)
                           * (Precision(1) - point.s) - Precision(0.25) * h_rt_5;
    const Precision h_rt_4 = Precision(0.25) * (Precision(1) + r_a)
                           * (Precision(1) - point.s) - Precision(0.25) * h_rt_5;

    const Precision h_st_1 = Precision(0.25) * (Precision(1) + s_a)
                           * (Precision(1) + point.r) - Precision(0.25) * h_st_5;
    const Precision h_st_2 = Precision(0.25) * (Precision(1) + s_a)
                           * (Precision(1) - point.r) - Precision(0.25) * h_st_5;
    const Precision h_st_3 = Precision(0.25) * (Precision(1) - s_a)
                           * (Precision(1) - point.r) - Precision(0.25) * h_st_5;
    const Precision h_st_4 = Precision(0.25) * (Precision(1) - s_a)
                           * (Precision(1) + point.r) - Precision(0.25) * h_st_5;

    // The fifth rt and st tensor terms use the physical basis at the element
    // center. Only their scalar coefficients are obtained by averaging the
    // corresponding RA/RB and SA/SB sampling values.
    const ReferencePoint* center_ptr =
        cached_reference_point(Precision(0), Precision(0));

    logging::error(center_ptr != nullptr,
                   "FRTShellS8: missing MITC8 center reference point");

    const ReferencePoint& center = *center_ptr;

    // Return the two target-natural engineering shear components generated by
    // one unit physical sampling tensor
    //
    //     0.5 * (g^alpha tensor n + n tensor g^alpha),
    //
    // where g^alpha is the selected contravariant sampling tangent and n is
    // the sampling-point reference normal. This retains the complete physical
    // orientation of every boundary and center tensor on curved elements.
    const auto shear_tensor_to_target = [&](const ReferencePoint& sample,
                                            Index                 direction) {
        const auto dual = dual_vectors(sample);

        const Vec3 contravariant = dual[static_cast<std::size_t>(direction)];
        const Vec3 sample_normal = sample.basis.col(2);
        const Vec3 target_normal = point.basis.col(2);

        Vec2 transformation;

        for (Index component = 0; component < 2; ++component) {
            const Vec3 target_tangent = point.X_rs.col(component);

            transformation(component) =
                  target_tangent.dot(contravariant)
                * sample_normal.dot(target_normal)
                + target_tangent.dot(sample_normal)
                * contravariant.dot(target_normal);
        }

        return transformation;
    };

    const Vec2 rt_1_to_target = shear_tensor_to_target(tying_points[ 8], 0);
    const Vec2 rt_2_to_target = shear_tensor_to_target(tying_points[ 9], 0);
    const Vec2 rt_3_to_target = shear_tensor_to_target(tying_points[10], 0);
    const Vec2 rt_4_to_target = shear_tensor_to_target(tying_points[11], 0);
    const Vec2 rt_5_to_target = shear_tensor_to_target(center,            0);

    const Vec2 st_1_to_target = shear_tensor_to_target(tying_points[14], 1);
    const Vec2 st_2_to_target = shear_tensor_to_target(tying_points[17], 1);
    const Vec2 st_3_to_target = shear_tensor_to_target(tying_points[16], 1);
    const Vec2 st_4_to_target = shear_tensor_to_target(tying_points[15], 1);
    const Vec2 st_5_to_target = shear_tensor_to_target(center,            1);
    ...
    shear_nat += h_rt_1 * rt_1_to_target * data.tying_strain_nat[ 8](gamma_r3);
    shear_nat += h_rt_2 * rt_2_to_target * data.tying_strain_nat[ 9](gamma_r3);
    shear_nat += h_rt_3 * rt_3_to_target * data.tying_strain_nat[10](gamma_r3);
    shear_nat += h_rt_4 * rt_4_to_target * data.tying_strain_nat[11](gamma_r3);
    shear_nat += h_rt_5 * rt_5_to_target * rt_mean;

    shear_nat += h_st_1 * st_1_to_target * data.tying_strain_nat[14](gamma_s3);
    shear_nat += h_st_2 * st_2_to_target * data.tying_strain_nat[17](gamma_s3);
    shear_nat += h_st_3 * st_3_to_target * data.tying_strain_nat[16](gamma_s3);
    shear_nat += h_st_4 * st_4_to_target * data.tying_strain_nat[15](gamma_s3);
    shear_nat += h_st_5 * st_5_to_target * st_mean;

    strain_nat.template segment<2>(gamma_r3) = shear_nat;
    ...
    if (B_nat) {
        ...
        Mat2x6N shear_B = Mat2x6N::Zero();

        shear_B.noalias() += rt_1_to_target * (h_rt_1 * data.tying_B_nat[ 8].row(gamma_r3));
        shear_B.noalias() += rt_2_to_target * (h_rt_2 * data.tying_B_nat[ 9].row(gamma_r3));
        shear_B.noalias() += rt_3_to_target * (h_rt_3 * data.tying_B_nat[10].row(gamma_r3));
        shear_B.noalias() += rt_4_to_target * (h_rt_4 * data.tying_B_nat[11].row(gamma_r3));
        shear_B.noalias() += rt_5_to_target * (h_rt_5 * rt_B_mean);

        shear_B.noalias() += st_1_to_target * (h_st_1 * data.tying_B_nat[14].row(gamma_s3));
        shear_B.noalias() += st_2_to_target * (h_st_2 * data.tying_B_nat[17].row(gamma_s3));
        shear_B.noalias() += st_3_to_target * (h_st_3 * data.tying_B_nat[16].row(gamma_s3));
        shear_B.noalias() += st_4_to_target * (h_st_4 * data.tying_B_nat[15].row(gamma_s3));
        shear_B.noalias() += st_5_to_target * (h_st_5 * st_B_mean);

        B_nat->template block<2, num_dofs>(gamma_r3, 0) = shear_B;
    }
 ...
 * and distributed to the eight compatible sampling tensors. The two target
 * shear weights are pulled back through the complete physical shear-tensor
 * interpolation and distributed to the boundary and interior averaging
...
void FRTShellS8::pull_back_mitc_resultants(
    ...
    // Build the same physical-sampling-tensor to target-natural map used by
    // apply_mitc_natural(). Its transpose is applied below to distribute the
    // two target shear weights back to each scalar tying-point component.
    const auto shear_tensor_to_target = [&](const ReferencePoint& sample,
                                            Index                 direction) {
        const auto dual = dual_vectors(sample);
    ...
    const Vec2 shear_weights = assumed_weights.template segment<2>(gamma_r3);

    tying_weights[ 8](gamma_r3) += h_rt_1 * rt_1_to_target.dot(shear_weights);
    tying_weights[ 9](gamma_r3) += h_rt_2 * rt_2_to_target.dot(shear_weights);
    tying_weights[10](gamma_r3) += h_rt_3 * rt_3_to_target.dot(shear_weights);
    tying_weights[11](gamma_r3) += h_rt_4 * rt_4_to_target.dot(shear_weights);
    tying_weights[12](gamma_r3) += Precision(0.5) * h_rt_5 * rt_5_to_target.dot(shear_weights);
    tying_weights[13](gamma_r3) += Precision(0.5) * h_rt_5 * rt_5_to_target.dot(shear_weights);

    tying_weights[14](gamma_s3) += h_st_1 * st_1_to_target.dot(shear_weights);
    tying_weights[17](gamma_s3) += h_st_2 * st_2_to_target.dot(shear_weights);
    tying_weights[16](gamma_s3) += h_st_3 * st_3_to_target.dot(shear_weights);
    tying_weights[15](gamma_s3) += h_st_4 * st_4_to_target.dot(shear_weights);
    tying_weights[18](gamma_s3) += Precision(0.5) * h_st_5 * st_5_to_target.dot(shear_weights);
    tying_weights[19](gamma_s3) += Precision(0.5) * h_st_5 * st_5_to_target.dot(shear_weights);
