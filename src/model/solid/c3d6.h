...
struct C3D6 : public SolidElement<6>{
    ...
    SurfacePtr surface(ID surface_id) override {
        ...
        switch (surface_id) {
            case 1: return std::make_unique<Surface3>(std::array<ID, 3>{node_ids[0], node_ids[1], node_ids[2]});  // Face 1: Triangle
            case 2: return std::make_unique<Surface3>(std::array<ID, 3>{node_ids[3], node_ids[4], node_ids[5]});  // Face 2: Triangle
            case 3: return std::make_unique<Surface4>(std::array<ID, 4>{node_ids[0], node_ids[1], node_ids[4], node_ids[3]});  // Face 3: Quadrilateral
            case 4: return std::make_unique<Surface4>(std::array<ID, 4>{node_ids[1], node_ids[2], node_ids[5], node_ids[4]});  // Face 4: Quadrilateral
            case 5: return std::make_unique<Surface4>(std::array<ID, 4>{node_ids[2], node_ids[0], node_ids[3], node_ids[5]});  // Face 5: Quadrilateral
