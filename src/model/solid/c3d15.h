...
struct C3D15 : public SolidElement<15>{
    ...
    SurfacePtr surface(ID surface_id) override {
        ...
        switch (surface_id) {
            case 1: return std::make_unique<Surface6>(std::array<ID, 6>{node_ids[0], node_ids[1], node_ids[2],
                                                                        node_ids[6], node_ids[7], node_ids[8]});  // Face 1: Triangle
            case 2: return std::make_unique<Surface6>(std::array<ID, 6>{node_ids[3], node_ids[4], node_ids[5],
                                                                         node_ids[10], node_ids[11], node_ids[12]});  // Face 2: Triangle
            case 3: return std::make_unique<Surface8>(std::array<ID, 8>{node_ids[0], node_ids[1], node_ids[4], node_ids[3],
                                                                         node_ids[6], node_ids[13], node_ids[9], node_ids[12]});  // Face 3: Quadrilateral
            case 4: return std::make_unique<Surface8>(std::array<ID, 8>{node_ids[1], node_ids[2], node_ids[5], node_ids[4],
                                                                         node_ids[7], node_ids[14], node_ids[10], node_ids[13]});  // Face 4: Quadrilateral
            case 5: return std::make_unique<Surface8>(std::array<ID, 8>{node_ids[2], node_ids[0], node_ids[3], node_ids[5],
