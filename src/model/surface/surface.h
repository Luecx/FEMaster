...
struct SurfaceInterface {
     ...
     * @return StaticVector<3> Global position corresponding to the local coordinate.
     *
     ...
     * @return StaticVector<2> The local coordinates (r, s) corresponding to the closest point on the surface element to point 'p'.
     *
     ...
     */
    StaticVector<2> global_to_local(const StaticVector<3>& p, const NodeData& node_coords_system, bool clip = false) const {

        constexpr int max_iter = 32;
        constexpr Precision eps = 1e-12;

        std::vector<StaticVector<2>> initial_guesses;
        ...
        StaticVector<2> best_coords = initial_guesses[0];
        ...
        for (const auto& initial_guess : initial_guesses) {
            ...
            for (Index iter = 0; iter < max_iter; iter++) {
                ...
                StaticVector<3> x_rs = StaticVector<3>::Zero();
                StaticVector<3> dx_dr = StaticVector<3>::Zero();
                StaticVector<3> dx_ds = StaticVector<3>::Zero();
                StaticVector<3> d2x_dr2 = StaticVector<3>::Zero();
                StaticVector<3> d2x_ds2 = StaticVector<3>::Zero();
                StaticVector<3> d2x_drds = StaticVector<3>::Zero();
            ...
            if (!clip || in_bounds({r,s})) {
                ...
                StaticVector<3> x_rs_final = this->local_to_global({r, s}, node_coords_system);
        ...
        if (clip) {
            // check the boundary
            StaticVector<2> best_coords_edge = closest_point_on_boundary(p, node_coords_global);
     ...
     *
     */
    virtual StaticVector<2> closest_point_on_boundary(const StaticVector<3>& global, const StaticMatrix<N, 3>& node_coords) const = 0;

    /**
     */
    virtual bool in_bounds(const StaticVector<2>& local) const = 0;
